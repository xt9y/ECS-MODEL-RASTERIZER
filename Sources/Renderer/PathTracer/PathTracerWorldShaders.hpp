#ifndef RW_ENGINE_RENDERER_PATHTRACER_WORLD_SHADERS_HPP
#define RW_ENGINE_RENDERER_PATHTRACER_WORLD_SHADERS_HPP
...
vec3 tracePath(vec3 origin, vec3 direction, inout uint state)
{
    ...
    for (int bounce = 0; bounce < bounce_count; ++bounce) {
        ...
        if (bounce + 1 >= bounce_count) break;
...
void main()
{
    ...
    int samples = clamp(uSamplesThisFrame, 1, 2);
