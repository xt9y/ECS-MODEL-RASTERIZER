...
void mergeReservoir(inout ReservoirData result,ReservoirData candidate,float scale,inout uint rng){float cm=candidate.sample_position_m.w,csum=candidate.radiance_weight.w;if(cm<=0.0||csum<=0.0)return;float capped_m=min(cm,20.0),weight=csum*scale*(capped_m/max(cm,1.0));if(weight<=0.0)return;float old_sum=result.radiance_weight.w,new_sum=old_sum+weight;if(old_sum<=0.0||randomFloat(rng)<weight/max(new_sum,1.0e-8)){result.sample_position_m.xyz=candidate.sample_position_m.xyz;result.radiance_weight.rgb=candidate.radiance_weight.rgb;}result.sample_position_m.w=min(max(result.sample_position_m.w,0.0)+capped_m,20.0);result.radiance_weight.w=new_sum;}
)GLSL";
...
void main(){ivec2 pixel=ivec2(gl_GlobalInvocationID.xy);if(any(greaterThanEqual(pixel,uResolution)))return;uint index=uint(pixel.y*uResolution.x+pixel.x);SurfaceData current=current_surfaces[index];ReservoirData result=temporal_reservoirs[index];if(!validSurface(current)){spatial_reservoirs[index]=result;return;}uint rng=hashUint(index*1597u^uint(uFrameIndex)*5171u^0xb5297a4du);int rotation=uFrameIndex&7;for(int s=0;s<4;++s){ivec2 np=pixel+OFFSETS[(rotation+s*2)&7];if(any(lessThan(np,ivec2(0)))||any(greaterThanEqual(np,uResolution)))continue;uint ni=uint(np.y*uResolution.x+np.x);if(!geometryCompatible(current,current_surfaces[ni]))continue;mergeReservoir(result,temporal_reservoirs[ni],0.5,rng);}spatial_reservoirs[index]=result;}
)GLSL";
